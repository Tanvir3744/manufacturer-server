...
async function run() {
    try {
        ...
        app.post('/orders', async (req, res) => {
            const tool = req.body;
            const query = { productName: tool.productName, email: tool.email }
            const exists = await purchasedCollection.findOne(query);
            if (exists) {
                return res.send({ success: false, tool: exists })
            }
            const result = await purchasedCollection.insertOne(tool);
        ...
        app.get('/purchase', async (req, res) => {
            ...
            const purchase = await purchasedCollection.find(query).toArray();
            res.send(purchase);
        });
